...
The refs are read from `SB_REACT_URL` and `SB_SVELTE_URL` (which default to `http://localhost:6007` and `http://localhost:6008` respectively), and the deployed build bakes in `react.components.scottylabs.org` and `svelte.components.scottylabs.org`.
